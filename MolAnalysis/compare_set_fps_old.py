...
#Get the corresponding dataseries
df1 = jl.load(PATH_1)
df2 = jl.load(PATH_2)
#-------------------------------------
...
for molfp in df1[fptype]:
    sim_df[1] = df2[fptype].apply(lambda x:DataStructs.FingerprintSimilarity(x, molfp, metric=DataStructs.TanimotoSimilarity)
    	if(np.all(pd.notnull(x))) else np.NaN)

jl.dump(sim_df, outfile,compress=4)
