...
def plotMw(dataframe,foldername):
    ...
    mwplot = sns.histplot(dataframe.droplevel(0, axis=1), x='Mw', bins=50, stat='percent')
...
def plotNumberOfHeavyAtoms(dataframe,foldername):
    ...
    nAtoms = sns.histplot(dataframe.droplevel(0, axis=1), x='# heavy atoms', bins=25, discrete=True)
